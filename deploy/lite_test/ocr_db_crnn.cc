...
void RunRecModel(std::vector<std::vector<std::vector<int>>> boxes, cv::Mat img,
                 ...
                 std::vector<std::string> charactor_dict, double &rec_time) {
  ...
  //  std::chrono::duration<double> duration_rec;
  for (int i = boxes.size() - 1; i >= 0; i--) {
    ...
    //    auto start_rec = GetCurrentUS();
    predictor_crnn->Run();
    auto end_rec = std::chrono::system_clock::now();
    //    auto end_rec = GetCurrentUS();

    //    rec_time += double((end_rec - start_rec) / 1000);
    auto duration_rec = std::chrono::duration_cast<std::chrono::microseconds>(
        end_rec - start_rec);
    rec_time += double(duration_rec.count()) *
...
RunDetModel(std::shared_ptr<PaddlePredictor> predictor, cv::Mat img,
            std::map<std::string, double> Config, double &det_time) {
  // Read img
  ...
  det_time += double(duration.count()) *
              std::chrono::microseconds::period::num /
              std::chrono::microseconds::period::den;
...
std::shared_ptr<PaddlePredictor> loadModel(std::string model_file,
  ...
  std::cout << "set threads as: " << threads << std::endl;
...
int main(int argc, char **argv) {
  ...
  int is_flag = std::stoi(argv[5]);
  int threads = std::stoi(argv[6]);

  //// load config from txt file
  auto Config = LoadConfigTxt("./config.txt");
  auto det_predictor = loadModel(det_model_file, threads);
  auto rec_predictor = loadModel(rec_model_file, threads);
  ...
  if (is_flag >= 1) {
    ...
    double det_time;
    auto start = std::chrono::system_clock::now();
    auto boxes = RunDetModel(det_predictor, srcimg, Config, det_time);

    std::vector<std::string> rec_text;
    std::vector<float> rec_text_score;
    double rec_times = 0.;
    RunRecModel(boxes, srcimg, rec_predictor, rec_text, rec_text_score,
                charactor_dict, rec_times);
    std::cout << "mean time per box: " << rec_times / boxes.size() << std::endl;
    std::cout << "mean time per image: " << rec_times << std::endl;
  ...
  if (is_flag < 1) {
    ...
    for (int i = 0; i < 9000; i++) {
      std::string img_name = img_path + "image_" + std::to_string(i) + ".jpg";

      cv::Mat srcimg = cv::imread(img_name, cv::IMREAD_COLOR);
      if (int(srcimg.channels()) == int(4))
      ...
      std::vector<std::vector<std::vector<int>>> boxes;
      try {
        boxes = RunDetModel(det_predictor, srcimg, Config, det_time);
      } catch (cv::Exception) {
        std::cout << "exception img: " << img_name << std::endl;
        continue;
      }
      if (boxes.size() < 1)
        continue;
      ...
      rec_time = 0.;
      RunRecModel(boxes, srcimg, rec_predictor, rec_text, rec_text_score,
                  charactor_dict, rec_time);
      rec_times += rec_time;
      rec_count += boxes.size();

      auto end_all = std::chrono::system_clock::now();
      auto duration_all = std::chrono::duration_cast<std::chrono::microseconds>(
          end_all - start_all);

      run_time += double(duration_all.count()) *
                  std::chrono::microseconds::period::num /
                  std::chrono::microseconds::period::den;

      std::cout << i << "\t"
                << "mean det time per img: " << det_time
                << "\t mean rec time per box: " << rec_time / boxes.size()
                << std::endl;
      //      //// print recognized text
      //      for (int i = 0; i < rec_text.size(); i++) {
      //        std::cout << i << "\t" << rec_text[i] << "\t" <<
      //        rec_text_score[i]
      //                << std::endl;
      //      }
    }
    std::cout << "mean det time: " << det_times / det_count
              << "\nmean rec time: " << rec_times / rec_count << std::endl;
    std::cout << "mean run(det+rec)_time: " << run_time / det_count
              << std::endl;
    std::cout << "run img num: " << det_count << std::endl;
  }
